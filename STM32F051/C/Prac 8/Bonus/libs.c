...
void lib_init_TIM14(uint32_t prescaler, uint32_t arr){
	...
	*(uint32_t*)0xE000E100 |= 0x80000;		//0b10000000000000000000 position 19
}

void lib_TIM14_update_ARR(uint32_t arr){
	TIM14->ARR = arr;
}

void lib_TIM14_update_PSC(uint32_t psc){
	TIM14->PSC = psc;
}

void lib_TIM14_ACK(void){
	TIM14->SR &= ~TIM_SR_UIF;
}
...
void lib_init_TIM6(uint32_t prescaler, uint32_t arr){
	...
	*(uint32_t*)0xE000E100 |= 0x20000;	//0b100000000000000000 Position 17
