...
main_loop:
	...
	MOVS R4, #0b100
	ANDS R4, R3, R4
	BEQ reverse		@ if held go to reverse logic, if not continue
	
no_reset:
	MOVS R4, #0b1000    	@ Is SW3 held?
	ANDS R4, R4, R3
	BEQ main_loop       	@ if so, loop without doing any work (freeze)
	LDR R5, LONG_DELAY_CNT  @ assume long delay
	MOVS R4, #0b10      	@ Is SW1 held?
	ANDS R4, R3, R4
...
delay_loop:
	...
	ANDS R3, R3, R4
	BNE write_leds        	@ if not, dont do anymore adding
